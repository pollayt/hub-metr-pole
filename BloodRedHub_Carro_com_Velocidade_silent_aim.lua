...
-- Silent aim: redireciona Mouse.Hit/Mouse.Target para a cabeca do alvo dentro do FOV.
-- Requer um executor que suporte hookmetamethod/newcclosure. Se o jogo nao usa Mouse.Hit,
-- o tiro precisa ser redirecionado no RemoteEvent especifico da arma.
local SilentAimInstalled=false
local SilentAimOldIndex=nil

local function getSilentTarget()
	if not Aim.Silent then return nil end
	return getAimTarget()
end

local function installSilentAim()
	if SilentAimInstalled then return true end
	if not hookmetamethod or not newcclosure then
		notify("silent aim","executor sem suporte a hookmetamethod","x")
		return false
	end

	local mouse=LP:GetMouse()
	SilentAimOldIndex=getrawmetatable and getrawmetatable(mouse) and getrawmetatable(mouse).__index or nil
	if not SilentAimOldIndex then
		notify("silent aim","nao foi possivel acessar Mouse.__index","x")
		return false
	end

	local old
	old=hookmetamethod(mouse,"__index",newcclosure(function(self,key)
		if self==mouse and Aim.Silent then
...
local function setSilentAim(v)
	if v then
		if installSilentAim() then
			Aim.Silent=true
			notify("silent aim","ativado: tiros priorizam a cabeca dentro do FOV","crosshair")
		end
	else
		Aim.Silent=false
		notify("silent aim","desativado","x")
	end
end
...
MainTab:Toggle({
	Title="silent aim",
	Desc="ao atirar, redireciona Mouse.Hit/Target para a cabeca dentro do FOV",
